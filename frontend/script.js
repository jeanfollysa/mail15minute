...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function startTimer() {
    clearInterval(timerInterval); // Arrêter le minuteur précédent
  ...
  // Lorsque l'email est généré
  generateBtn.addEventListener("click", async () => {
    console.log("🔁 Bouton 'Générer un email' cliqué !");
    try {
      const res = await fetch("http://localhost:3000/create-email");
      ...
      // Réinitialiser et démarrer la minuterie
      startTimer();
    } catch (err) {
  ...
  async function fetchInbox(email) {
    ...
    try {
      const res = await fetch(`http://localhost:3000/inbox/${email}`);
      const data = await res.json();
